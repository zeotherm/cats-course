...
object Essentials {
  ...
  val anotherCheckerboard = for {
    n <- List(1, 2, 3)
    c <- List('a', 'b', 'c')
  } yield (n, c) // equivalent expression as checkerboard

  // options and try
  val anOption: Option[Int] = Option(/* Something that might be null */ 3) // Some(3)
  val doubledOption: Option[Int] = anOption.map(_ * 2)

  val anAttempt = Try(/* something that might throw */ 42) // Success(42) or a Failure
  ...
  val aFuture = Future {
    // a bit of code
    42
  } // code block is the argument of the companion objects apply method

  // wait for completion (async)
  aFuture.onComplete {
    case Success(value) => println(s"The async meaning of life is $value")
    case Failure(ex) => println(s"Meaning of value failed: $ex")
  }
